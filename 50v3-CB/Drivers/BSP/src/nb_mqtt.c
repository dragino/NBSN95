...
NB_TaskStatus nb_QSSLCFG_set(const char* param)
{
	...
	strcat(buff,AT QSSLCFG "=\"seclevel\",0,");	
...
{	
	try_num = 4;
	NBTask[_AT_SNI].set(param);
	
	if(nb_at_send(&NBTask[_AT_SNI]) == NB_CMD_SUCC)
		nb_cmd_status = NB_CMD_SUCC;
	else
		nb_cmd_status = NB_CMD_FAIL;
...
NB_TaskStatus nb_QMTCFGSSL_set(const char* param)
{
	...
	strcat(buff,AT QMTCFG "=\"ssl\",0,1,0\r\n");	
...
{	
	try_num = 4;
	NBTask[_AT_CACERT].set(param);
	
	if(nb_at_send(&NBTask[_AT_CACERT]) == NB_CMD_SUCC)
		nb_cmd_status = NB_CMD_SUCC;
	else
		nb_cmd_status = NB_CMD_FAIL;
...
NB_TaskStatus nb_CACERT_set(const char* param)
{
	...
	strcat(buff,AT QSSLCFG "=\"cacert\",0,\"cacert.pem\"\r\n");	
...
{	
	try_num = 4;
	NBTask[_AT_CLICERT].set(param);
	
	if(nb_at_send(&NBTask[_AT_CLICERT]) == NB_CMD_SUCC)
		nb_cmd_status = NB_CMD_SUCC;
	else
		nb_cmd_status = NB_CMD_FAIL;
...
NB_TaskStatus nb_CLICERT_set(const char* param)
{
	...
	strcat(buff,AT QSSLCFG "=\"clientcert\",0,\"client.pem\"\r\n");		
...
{	
	try_num = 4;
	NBTask[_AT_CLIKEY].set(param);
	
	if(nb_at_send(&NBTask[_AT_CLIKEY]) == NB_CMD_SUCC)
		nb_cmd_status = NB_CMD_SUCC;
	else
		nb_cmd_status = NB_CMD_FAIL;
...
NB_TaskStatus nb_CLIKEY_set(const char* param)
{
	...
	strcat(buff,AT QSSLCFG "=\"clientkey\",0,\"user_key.pem\"\r\n");	
...
{
	try_num = 4;
	NBTask[_AT_MQTT_PUBEX].set(NULL);
...
NB_TaskStatus nb_MQTT_pub_set(const char* param)
{
	memset(buff,0,sizeof(buff));
	if(mqtt_qos==2)
	strcat(buff,AT QMTPUBEX"=0,1,2,0,\"");
	else if(mqtt_qos==0)	
	strcat(buff,AT QMTPUBEX"=0,0,0,0,\"");
	else if(mqtt_qos==1)	
	strcat(buff,AT QMTPUBEX"=0,1,1,0,\"");
	...
	ATSendStr  = buff;
	len_string = strlen(ATSendStr);
	user_main_debug("NBTask[_AT_MQTT_PUBEX].ATSendStr:%s",ATSendStr);
	return nb_cmd_status;
}
...
NB_TaskStatus nb_MQTT_disc_run(const char* param)
{
	...
	try_num = 3;
	NBTask[_AT_MQTT_DISC].set(NULL);

		while(try_num--)
	{	
	if(nb_at_send(&NBTask[_AT_MQTT_DISC]) == NB_CMD_SUCC)
	{
		nb_cmd_status = NB_CMD_SUCC;
		break;		
	}
...
{
	user_main_debug("NBTask[_AT_MQTT_DISC].ATSendStr:%s",ATSendStr);
...
{
	ATSendStr  = NULL;
	ATSendStr = AT QMTCLOSE"=0" NEWLINE;
	len_string = sizeof(AT QMTCLOSE"=0" NEWLINE) - 1;
...
{
	NBTask[_AT_OTASEND].set(NULL);

	if(nb_at_send(&NBTask[_AT_OTASEND]) == NB_CMD_SUCC)
	{
		nb_cmd_status = NB_CMD_SUCC;
	}
	else
...
//Ask if the process has failed 
	if((strstr((char*)nb.usart.data,QMTSTAT5) != NULL) ||(strstr((char*)nb.usart.data,QMTDISC": 0,0") != NULL))
	{
		nb_cmd_status = NB_DISC_SUCC;
	}
	else if(strstr((char*)nb.usart.data,QMTSTAT) != NULL)
	{
