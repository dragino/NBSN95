...
#ifdef NB_1D	
	#if defined NB_NS
	printf("SN50V3-CS-1D,%s\r\n",AT_VERSION_STRING);
	#else	
	printf("SN50V3-CB-1D,%s\r\n",AT_VERSION_STRING);
	#endif
#else	
	#if defined NB_NS
...
ATEerror_t at_dnstimer_get(const char *param)
{
	if(keep)
		printf("AT+BKDNS=");
	printf("%d,%d,",sys.ddns_flag,sys.ddns_time);
...
ATEerror_t at_dnstimer_set(const char *param)
{
	...
	if(tem > 0x01||countchar(pos,',')==0 ||tem2 > 0)
	{
		return AT_PARAM_ERROR;
	}
	if(countchar(pos,',')==2)
...
/************** 			AT+GDNS		 **************/
ATEerror_t at_gdns_get(const char *param)
{
	if(keep)
		printf("AT+GDNS=");
	printf("%d\r\n",sys.dns_time);
...
ATEerror_t at_gdns_set(const char *param)
{
	char* pos = strchr(param,'=');
	uint32_t tdc = atoi((param+(pos-param)+1));
	if(tdc > 1)
...
ATEerror_t at_uri5_set(const char *param)
{
	...
	memset(user.uri5,0,sizeof(user.uri5));
	memcpy(user.uri5,(param+(pos-param)+1),strlen((param+(pos-param)+1)));	
...
ATEerror_t at_qsw_run(const char *param)
{
	...
	else
	{
	   ...
	   bg95module_off();
		...
		user_main_printf("power off");
	}
  return AT_OK;
}

/************** 			AT+GNSST		 **************/
ATEerror_t at_gnsst_get(const char *param)
{
	if(keep)
		printf("AT+GNSST=");
	printf("%d\r\n",gnss_timer);
...
/************** 			AT+GTDC		 **************/
ATEerror_t at_gtdc_get(const char *param)
{
	if(keep)
		printf("AT+GTDC=");
	printf("%d\r\n",sys.gps_tdc);
...
ATEerror_t at_gtdc_set(const char *param)
{
	char* pos = strchr(param,'=');
	uint16_t gtdc = atoi((param+(pos-param)+1));
	if(gtdc > 0xFFFF || gtdc<1)
	{
		return AT_PARAM_ERROR;
	}
	sys.gps_tdc = gtdc;
...
/************** 			AT+QBAND		**************/
ATEerror_t at_qband_get(const char *param)
{
	if(keep)
		printf(AT QBAND"=");
	printf("%s\r\n",user.qband);
...
ATEerror_t at_qband_set(const char *param)
{
	space_fun((char*)param);
	char* pos = strchr(param,'=');
	if(strlen(param) - (pos-param)-1 >53)
	{
		return AT_PARAM_ERROR;
	}
	if(countchar(pos,',')!=1)
	...
	memset(user.qband,0,sizeof(user.qband));
	memcpy(user.qband,(param+(pos-param)+1),strlen((param+(pos-param)+1)));		
	return AT_OK;
}
...
ATEerror_t at_iotmod_set(const char *param)
{
	char* pos = strchr(param,'=');
	uint8_t tdc = atoi((param+(pos-param)+1));
	if(tdc > 2)
	{
		return AT_PARAM_ERROR;
	}
...
void config_Get(void)
{
  ...
  add = FLASH_USER_START_ADDR_CONFIG;	
	...
	sensor.exit_count_pa4 = FLASH_read(add+4);
	sensor.exit_count_pa0 = FLASH_read(add+124);
	
	sensor.count_mode= FLASH_read(add+48)>>16& 0x000000FF;
	
	sys.inmod_pa4= FLASH_read(add)>>24 & 0x000000FF;
	if(sys.inmod_pa4 != 0 && sys.inmod_pa4 != 1 && sys.inmod_pa4 != 2 && sys.inmod_pa4 != 3)
		sys.inmod_pa4 = 0;
	EX_GPIO_Init_pa4(sys.inmod_pa4);

	sys.inmod_pa0= FLASH_read(add)>>16 & 0x000000FF;
	if(sys.inmod_pa0 != 0 && sys.inmod_pa0 != 1 && sys.inmod_pa0 != 2 && sys.inmod_pa0 != 3)
		sys.inmod_pa0 = 0;
	EX_GPIO_Init_pa0(sys.inmod_pa0);
	sys.gps_tdc = FLASH_read(add)& 0x0000FFFF;
	if(sys.gps_tdc == 0)
		sys.gps_tdc =24;
...
//	noud_flags=FLASH_read(add+44)>>8&0xFF;
  ...
  sys.dns_time = FLASH_read(add+48)>>8 &0xFF;	
	...
	sys.iotmode= FLASH_read(add+48) &0xFF;	
...
//	mqtt_qos_flags = FLASH_read(add+112)>>24 &0xFF;	
  ...
  ipv46 = FLASH_read(add+120) &0xFF;
	...
	gnss_timer = FLASH_read(add+120)>>16 &0xFFFF;
	if(gnss_timer == 0)
		gnss_timer = 60;
	
  gps_flag = FLASH_read(add+120)>>8 &0xFF;
	
  sys.platform	= FLASH_read(add+48)>>24 &0xFF;	
	
	sys.downlink_1t= FLASH_read(add+52)>>8 &0xFF;	

	sys.downlink_debug= FLASH_read(add+52) &0xFF;
	
  sys.ddns_time = FLASH_read(add+52)>>16 &0xFF;	
  ...
  sys.ddns_flag	= FLASH_read(add+52)>>24 &0xFF;		
	...
	add = FLASH_USER_COAP_URI5;
	for(uint8_t i=0,j=0;i<32;i++,j=j+4)
	{
		uint32_t temp = FLASH_read(add+i*4);
		user.uri5[j] 	= (temp>>24) & 0x000000FF;
		user.uri5[j+1] = (temp>>16) & 0x000000FF;
		user.uri5[j+2] = (temp>>8)  & 0x000000FF;
		user.uri5[j+3] = (temp)     & 0x000000FF;
	}
	if(strlen((char*)user.uri5) == 0)
	{
	...
	{
		uint32_t temp = FLASH_read(add+i*4);
		user.qband[j] 	= (temp>>24) & 0x000000FF;
		user.qband[j+1] = (temp>>16) & 0x000000FF;
		user.qband[j+2] = (temp>>8)  & 0x000000FF;
		user.qband[j+3] = (temp)     & 0x000000FF;
	}
