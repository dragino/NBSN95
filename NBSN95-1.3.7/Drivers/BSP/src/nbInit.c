...
case _AT_CFUNOFF:{
          ...
          HAL_RTC_GetDate(&hrtc, &RTC_DateStructure, RTC_FORMAT_BIN);	
	        ...
	        stm.tm_year=2000+RTC_DateStructure.Year-1900;  
         	stm.tm_mon=RTC_DateStructure.Month-1;  
        	stm.tm_mday=RTC_DateStructure.Date;  
        	stm.tm_hour=RTC_TimeStructure.Hours;  
        	stm.tm_min=RTC_TimeStructure.Minutes;  
        	stm.tm_sec=RTC_TimeStructure.Seconds;  
        	sensor.time_stamp = mktime(&stm);	
	        user_main_debug("time_stamp:%d",sensor.time_stamp);
...
case _AT_UPLOAD_START:{
				...
				if(sys.protocol == COAP_PRO)		
        	{
				...
				else if(sys.protocol == TCP_PRO)	{*task=_AT_TCP_OPEN;tcp_flags=1;}
