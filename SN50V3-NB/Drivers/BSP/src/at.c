...
ATEerror_t at_fdr_run(const char *param)
{
	...
	mqtt_qos = 0; 
	sys.sht_noud = 8;
	general_parameters[12]=sys.ddns_flag<<16;
  general_parameters[11]=qband_flag<<24  |sys.sht_noud;
...
/************** 			AT+CACERT		**************/
ATEerror_t at_cacert_run(const char *param)
{
	...
	nb_at_run(NULL);
	if(nb_QSSLCACERT_run(NULL)==NB_CMD_SUCC)
	{
	  printf("Please enter CA certificate:\r\n");
	}
  else
  {
	  printf("Failed to configure CA certificate.\r\n");	
	}	
	
  return AT_OK;		
}
/************** 			AT+CLIENTCERT		**************/
ATEerror_t at_clientcert_run(const char *param)
{
	if(nb.uplink_flag != no_status)
	{
		printf("The device is busy.\r\n");		
    return AT_BUSY_ERROR;	
	  ...
	  nb.uplink_flag = writing;
	  nb_at_run(NULL);
	if(nb_QSSLCCERT_run(NULL)==NB_CMD_SUCC)
	{
	  printf("Please enter client certificate:\r\n");	
	}
  else
  {	
	  printf("Failed to configure client certificate.\r\n");	
	}	
	
  return AT_OK;	
}
/************** 			AT+CLIENTKEY		**************/
ATEerror_t at_clientkey_run(const char *param)
{
	if(nb.uplink_flag != no_status)
	{
		printf("The device is busy.\r\n");
    ...
    nb.uplink_flag = writing;	
	  nb_at_run(NULL);
	if(nb_QSSLKEY_run(NULL)==NB_CMD_SUCC)
	{
	  printf("Please enter client private key:\r\n");	
	}
  else
  {
	  printf("Failed to configure client certificate.\r\n");	
...
void config_Set(void)
{
	...
	general_parameters[11]=qband_flag<<24  |sys.tr_time<<16 | sys.sht_noud;
...
//	FLASH_program(FLASH_USER_START_COAP,coap_parameters, sizeof(coap_parameters)/4);
	...
	for(uint8_t i=0,j=0;i<strlen((char*)user.otaver);i=i+4,j++)
			ota_parameters[14+j]=user.otaver[i+0]<<24 | user.otaver[i+1]<<16 | user.otaver[i+2]<<8 | user.otaver[i+3];
...
//	noud_flags=FLASH_read(add+20)>>8&0xFF;
...
//	 sys.retransmission_flag = FLASH_read(add+116) &0xFF;	
  ...
  sys.platform	= FLASH_read(add+48)>>24 &0xFF;	
	...
	for(uint8_t i=0,j=0;i<2;i++,j=j+4)
	{
		uint32_t temp      = FLASH_read(add+(i+14)*4);		
