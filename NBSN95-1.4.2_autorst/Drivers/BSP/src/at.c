...
static uint32_t mqtt_parameters_pwd[32]={0};
...
{
	char* pos = strchr(param,'=');
	if(strlen(param) - (pos-param)-1 >36)
	{
		return AT_PARAM_ERROR;
	}
...
/************** 			AT+AUTORST		 **************/
ATEerror_t at_autorst_get(const char *param)
{
	if(keep)
		printf("AT+AUTORST=");
	printf("%d\r\n",sys.auto_time);
...
ATEerror_t at_autorst_set(const char *param)
{
	char* pos = strchr(param,'=');
...
void config_Set(void)
{
	...
	general_parameters[20]=sys.nband_flag<<24 |sys.auto_time<<16| sys.csq_time<<8|controlMCU_flag;
	...
	for(uint8_t i=0,j=0;i<strlen((char*)user.dns_add);i=i+4,j++)
			general_parameters[21+j]=user.dns_add[i+0]<<24 | user.dns_add[i+1]<<16 | user.dns_add[i+2]<<8 | user.dns_add[i+3];
...
void config_Get(void)
{
  ...
  sys.auto_time = FLASH_read(add+80)>>16 &0xFF;	
	if(sys.auto_time==0)
		sys.auto_time =24;	
	...
	for(uint8_t i=0,j=0;i<9;i++,j=j+4)
	{
		uint32_t temp  = FLASH_read(add+i*4);
	...
	if(strlen((char*)user.apn) == 0)
	{
		...
		add = FLASH_USER_START_ADDR_CONFIG + 0x04*21;
	for(uint8_t i=0,j=0;i<4;i++,j=j+4)
	{
		uint32_t temp  = FLASH_read(add+i*4);
